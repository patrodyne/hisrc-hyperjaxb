...
public class SingleMarshallingReferenceField extends AbstractWrappingField
{
		...
		CPropertyInfo prop, CPropertyInfo core, String contextPath, boolean _final)
	{
		...
		this.contextPath = context.implClass.field(
			JMod.PUBLIC | JMod.STATIC | (_final ? JMod.FINAL : JMod.NONE),
	...
	protected JExpression wrap(JExpression target)
	{
		...
		// final CNonElement type = elementInfo.getProperty().ref().iterator().next();
		final JClass scope = getScope(elementInfo.getScope());
		final QName name = elementInfo.getElementName();
		return codeModel.ref(JAXBContextUtils.class).staticInvoke("unmarshalJAXBElement")
			.arg(contextPath)
	...
	{
		return codeModel.ref(JAXBContextUtils.class).staticInvoke("marshalJAXBElement")
			.arg(contextPath)
			.arg(source);
	}
