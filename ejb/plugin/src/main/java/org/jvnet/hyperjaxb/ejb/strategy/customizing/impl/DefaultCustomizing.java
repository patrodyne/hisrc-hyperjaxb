...
public class DefaultCustomizing implements Customizing
{
	...
	{
		final List<CPluginCustomization> customizations = CustomizationUtils.findCustomizations(classInfo, name);
		return unmarshalCustomizations(customizations);
	}

	private List<CPluginCustomization> findCustomizations(CTypeInfo typeInfo, QName name)
	{
	...
	{
		final CPluginCustomization pluginCustomization = CustomizationUtils.addCustomization(customizable,
			Customizations.getContext(), name, customization);
		this.customizationsMap.put(pluginCustomization, customization);
	}

	private <T> Collection<T> unmarshalCustomizations(final Collection<CPluginCustomization> customizations)
		throws AssertionError
	{
		final List<T> unmarshalledCustomizations = new ArrayList<T>(customizations.size());
		for (CPluginCustomization customization : customizations)
		{
			unmarshalledCustomizations.add(this.<T> unmarshalCustomization(customization, null, null));
		}
		return unmarshalledCustomizations;
	}

	@SuppressWarnings("unchecked")
	private <T> T unmarshalCustomization(final CPluginCustomization customization, T defaultValue, Merge<T> merge)
		throws AssertionError
	{
	...
	{
		final Persistence cPersistence = findCustomization(model, Customizations.PERSISTENCE_ELEMENT_NAME);
	...
	public GeneratedId getGeneratedId(CClassInfo classInfo)
	{
		...
		final GeneratedId defaultId = (GeneratedId) persistence.getDefaultGeneratedId().copyTo(new GeneratedId());
		final GeneratedId id;
		if (CustomizationUtils.containsCustomization(classInfo, Customizations.GENERATED_ID_ELEMENT_NAME))
		{
			id = findCustomization(classInfo, Customizations.GENERATED_ID_ELEMENT_NAME, defaultId,
	...
	public GeneratedClass getGeneratedClass(CPropertyInfo propertyInfo)
	{
		...
		if (CustomizationUtils.containsCustomization(propertyInfo, Customizations.GENERATED_CLASS_ELEMENT_NAME))
		{
			generatedClass = findCustomization(propertyInfo, Customizations.GENERATED_CLASS_ELEMENT_NAME);
		}
		else
		{
			generatedClass = null;
	...
	public GeneratedProperty getGeneratedProperty(CPropertyInfo propertyInfo)
	{
		...
		if (CustomizationUtils.containsCustomization(propertyInfo, Customizations.GENERATED_PROPERTY_ELEMENT_NAME))
		{
			generatedProperty = findCustomization(propertyInfo, Customizations.GENERATED_PROPERTY_ELEMENT_NAME);
		}
		else
		{
			generatedProperty = getGeneratedProperty((CClassInfo) propertyInfo.parent(), propertyInfo.getName(true));
		}
	...
	public GeneratedProperty getGeneratedProperty(CClassInfo classInfo, String propertyName)
	{
		...
		if (classInfo != null
			&& CustomizationUtils.containsCustomization(classInfo, Customizations.GENERATED_PROPERTY_ELEMENT_NAME))
		{
			final Collection<GeneratedProperty> generatedProperties;
			generatedProperties = findCustomizations(classInfo, Customizations.GENERATED_PROPERTY_ELEMENT_NAME);
	...
	public Id getId(CPropertyInfo property)
	{
		...
		final Id id;
		if (CustomizationUtils.containsCustomization(property, Customizations.ID_ELEMENT_NAME))
		{
			id = findCustomization(property, Customizations.ID_ELEMENT_NAME, defaultId, this.<Id> merge());
		}
		else
		{
			id = defaultId;
	...
	public EmbeddedId getEmbeddedId(CPropertyInfo property)
	{
		...
		final EmbeddedId id;
		if (CustomizationUtils.containsCustomization(property, Customizations.EMBEDDED_ID_ELEMENT_NAME))
		{
			id = findCustomization(property, Customizations.EMBEDDED_ID_ELEMENT_NAME, defaultId,
	...
	public Version getVersion(CPropertyInfo property)
	{
		...
		final Version defaultVersion = (Version) persistence.getDefaultVersion().copyTo(new Version());
		final Version version;
		if (CustomizationUtils.containsCustomization(property, Customizations.VERSION_ELEMENT_NAME))
		{
			version = findCustomization(property, Customizations.VERSION_ELEMENT_NAME, defaultVersion,
				this.<Version> merge());
		}
		else
		{
	...
	public GeneratedVersion getGeneratedVersion(CClassInfo classInfo)
	{
		...
		final GeneratedVersion defaultGeneratedVersion = (GeneratedVersion) persistence.getDefaultGeneratedVersion()
			.copyTo(new GeneratedVersion());
		...
		{
			generatedVersion = findCustomization(classInfo, Customizations.GENERATED_VERSION_ELEMENT_NAME,
	...
	public Basic getDefaultBasic(CPropertyInfo property)
		throws AssertionError
	{
		final Persistence persistence = getModelCustomization(property);
		...
		{
			defaultBasic = (Basic) persistence.getDefaultBasic().copyTo(new Basic());
		}
		else
		{
			final List<QName> typeNames = TypeUtils.getTypeNames(schemaComponent);
			Basic basic = null;
			for (Iterator<QName> typeNameIterator = typeNames.iterator(); typeNameIterator.hasNext() && basic == null;)
			{
				final QName typeName = typeNameIterator.next();
				final SingleProperty singleProperty = getDefaultSingleProperty(persistence, typeName);
			...
			else
			{
				...
				mergeFrom(defaultBasic, (Basic) persistence.getDefaultBasic().copyTo(new Basic()));
			}
		}
		...
		throws AssertionError
	{
		...
		if (schemaComponent == null)
		{
			defaultItem = (ElementCollection) persistence.getDefaultElementCollection().copyTo(new ElementCollection());
		}
		else
		{
			final List<QName> typeNames = TypeUtils.getTypeNames(schemaComponent);
			ElementCollection item = null;
			for (Iterator<QName> typeNameIterator = typeNames.iterator(); typeNameIterator.hasNext() && item == null;)
			{
				final QName typeName = typeNameIterator.next();
				final CollectionProperty collectionProperty = getDefaultCollectionProperty(persistence, typeName);
			...
			if (item == null)
			{
				defaultItem = (ElementCollection) persistence.getDefaultElementCollection()
					.copyTo(new ElementCollection());
			}
			else
			{
				defaultItem = (ElementCollection) item.copyTo(new ElementCollection());
				mergeFrom(defaultItem,
					(ElementCollection) persistence.getDefaultElementCollection().copyTo(new ElementCollection()));
			}
		}
	...
	{
		final Integer scale;
		final Long fractionDigits = SimpleTypeAnalyzer.getFractionDigits(property.getSchemaComponent());
		final Long totalDigits = SimpleTypeAnalyzer.getTotalDigits(property.getSchemaComponent());
	...
	public OneToMany getOneToMany(CPropertyInfo property)
	{
		...
		final OneToMany defaultOneToMany = (OneToMany) persistence.getDefaultOneToMany().copyTo(new OneToMany());
		if (CustomizationUtils.containsCustomization(property, Customizations.ONE_TO_MANY_ELEMENT_NAME))
		{
	...
	private void merge(final OneToMany cOneToMany, final OneToMany defaultOneToMany)
	{
	...
	public ManyToOne getManyToOne(CPropertyInfo property)
	{
		...
		final ManyToOne defaultManyToOne = (ManyToOne) persistence.getDefaultManyToOne().copyTo(new ManyToOne());
		final ManyToOne cmanyToOne;
		if (CustomizationUtils.containsCustomization(property, Customizations.MANY_TO_ONE_ELEMENT_NAME))
		{
	...
	private void merge(final ManyToOne cManyToOne, final ManyToOne defaultManyToOne)
	{
	...
	public OneToOne getOneToOne(CPropertyInfo property)
	{
		...
		final OneToOne defaultOneToOne = (OneToOne) persistence.getDefaultOneToOne().copyTo(new OneToOne());
		final OneToOne cOneToOne;
		if (CustomizationUtils.containsCustomization(property, Customizations.ONE_TO_ONE_ELEMENT_NAME))
		{
	...
	public ManyToMany getManyToMany(CPropertyInfo property)
	{
		...
		final ManyToMany defaultManyToMany = (ManyToMany) persistence.getDefaultManyToMany().copyTo(new ManyToMany());
		final ManyToMany cManyToMany;
		if (CustomizationUtils.containsCustomization(property, Customizations.MANY_TO_MANY_ELEMENT_NAME))
		{
	...
	public ElementCollection getElementCollection(CPropertyInfo property)
	{
		...
		if (CustomizationUtils.containsCustomization(property, Customizations.ELEMENT_COLLECTION_ELEMENT_NAME))
		{
			item = findCustomization(property, Customizations.ELEMENT_COLLECTION_ELEMENT_NAME, defaultItem,
				this.<ElementCollection> merge());
		}
		else
		{
			item = defaultItem;
	...
	public Entity getEntity(CClassInfo classInfo)
	{
		...
		final Entity defaultEntity = (Entity) persistence.getDefaultEntity().copyTo(new Entity());
		final Entity cEntity;
		if (CustomizationUtils.containsCustomization(classInfo, Customizations.ENTITY_ELEMENT_NAME))
		{
			cEntity = findCustomization(classInfo, Customizations.ENTITY_ELEMENT_NAME, defaultEntity,
				this.<Entity> merge());
		}
		else
		{
			addCustomization(classInfo, Customizations.ENTITY_ELEMENT_NAME, defaultEntity);
	...
	public Object getToOne(CPropertyInfo property)
	{
		...
		else if (CustomizationUtils.containsCustomization(property, Customizations.ONE_TO_ONE_ELEMENT_NAME))
		{
			return getOneToOne(property);
		}
		if (CustomizationUtils.containsCustomization(property, Customizations.EMBEDDED_ELEMENT_NAME))
		{
			return getEmbedded(property);
		}
		else
		{
			final ToOne defaultToOne = (ToOne) persistence.getDefaultToOne().copyTo(new ToOne());
			...
			else if (defaultToOne.getEmbedded() != null)
			{
				...
				final Embedded defaultEmbedded = persistence.getDefaultEmbedded();
	...
	public Object getToMany(CPropertyInfo property)
	{
		...
		else if (CustomizationUtils.containsCustomization(property, Customizations.ONE_TO_MANY_ELEMENT_NAME))
		{
			return getOneToMany(property);
		}
		else
		{
			final ToMany defaultToMany = (ToMany) persistence.getDefaultToMany().copyTo(new ToMany());
	...
	public MappedSuperclass getMappedSuperclass(CClassInfo classInfo)
	{
		...
		final MappedSuperclass defaultMappedSuperclass = (MappedSuperclass) persistence.getDefaultMappedSuperclass()
			.copyTo(new MappedSuperclass());
		final MappedSuperclass cMappedSuperclass;
		if (CustomizationUtils.containsCustomization(classInfo, Customizations.MAPPED_SUPERCLASS_ELEMENT_NAME))
		{
			cMappedSuperclass = findCustomization(classInfo, Customizations.MAPPED_SUPERCLASS_ELEMENT_NAME,
	...
	public Object getEntityOrMappedSuperclassOrEmbeddable(CClassInfo classInfo)
	{
		...
		else if (CustomizationUtils.containsCustomization(classInfo, Customizations.MAPPED_SUPERCLASS_ELEMENT_NAME))
		{
			return getMappedSuperclass(classInfo);
		}
		else if (CustomizationUtils.containsCustomization(classInfo, Customizations.EMBEDDABLE_ELEMENT_NAME))
		{
	...
	public Embeddable getEmbeddable(CClassInfo classInfo)
	{
		...
		final Embeddable defaultEmbeddable = (Embeddable) persistence.getDefaultEmbeddable().copyTo(new Embeddable());
		final Embeddable cEmbeddable;
		if (CustomizationUtils.containsCustomization(classInfo, Customizations.EMBEDDABLE_ELEMENT_NAME))
		{
			cEmbeddable = findCustomization(classInfo, Customizations.EMBEDDABLE_ELEMENT_NAME, defaultEmbeddable,
	...
	public Embedded getEmbedded(CPropertyInfo property)
	{
		...
		final Embedded defaultEmbedded = (Embedded) persistence.getDefaultEmbedded().copyTo(new Embedded());
		final Embedded embedded;
		if (CustomizationUtils.containsCustomization(property, Customizations.EMBEDDED_ELEMENT_NAME))
		{
			embedded = findCustomization(property, Customizations.EMBEDDED_ELEMENT_NAME, defaultEmbedded,
				this.<Embedded> merge());
		}
		else
		{
