...
public class DefaultCreateDefaultIdPropertyInfos implements CreateDefaultIdPropertyInfos
{
	...
	public QName getAttributeName(ProcessModel context, GeneratedId id)
	{
		...
		return attributeName != null ? attributeName : new QName(getPropertyName(context, id));
	}
	...
	public CNonElement getPropertyTypeInfo(ProcessModel context, GeneratedId id)
	{
		...
		Validate.notEmpty(javaType, "The hj:id/@javaType attribute must not be empty.");
		final QName schemaType = id.getSchemaType();
		Validate.notNull(schemaType, "The hj:id/@schemaType attribute must not be null.");
		try
		{
		...
		catch (ClassNotFoundException cnfex)
		{
			throw new IllegalArgumentException(
				"Class name [" + javaType + "] provided in the hj:id/@javaType attribute could not be resolved.",
				cnfex);
		}
	}
